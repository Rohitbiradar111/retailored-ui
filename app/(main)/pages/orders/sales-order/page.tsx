...
const SalesOrder = () => {
    ...
    const handleMeasurementValueChange = (id: string, value: string) => {
        setEditedMeasurements((prev) => prev.map((item) => (item.id === id ? { ...item, value } : item)));
    };
    ...
    const handleDelivered = async () => {
        ...
        try {
            await SalesOrderService.markOrderDelivered(selectedOrder.id, quantity);
    ...
    const handleCancelled = async () => {
        ...
        try {
            await SalesOrderService.markOrderCancelled(selectedOrder.id, quantity);
            ...
            <div className="grid">
                {orders.length > 0 ? (
                    orders.map((order, index) => (
                        <div key={order.id} className="col-12 md:col-6 lg:col-4" ref={index === orders.length - 1 ? lastOrderRef : null}>
                            <Card className="h-full">
                                <div className="flex flex-column gap-2">
                                    <div className="flex justify-content-between align-items-center">
                ...
                {listLoading ? (
                    ...
                    <div className="p-fluid mt-3">
                            ...
                            <div className="col-6">
                                <div className="field">
                                    ...
                                    <p className="m-0 font-medium">
                                        {selectedOrder.orderDetails?.some((item) => item.trial_date) ? formatDate(new Date(selectedOrder.orderDetails.find((item) => item.trial_date)?.trial_date || '')) : 'Not scheduled'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {selectedOrder.orderDetails.map((item) => (
                            <p key={item.id}>{item.material?.name || 'Not Available'}</p>
                        ))}
                        <Button label={showDetails ? 'Hide Details' : 'View Details'} onClick={() => setShowDetails((prev) => !prev)} className="mb-3" />

                        {showDetails &&
                            selectedOrder.orderDetails?.map((item) => (
                                <div key={item.id} className="mb-4 surface-50 p-3 border-2 border-round m-3">
                                    <div className="grid">
                                        <div className="col-6">
                                            <div className="field">
